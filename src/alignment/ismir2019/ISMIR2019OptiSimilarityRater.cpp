...
#include "ISMIR2019OptiSimilarityRater.h"
#include "ISMIR2019Symbol.h"
#include "ISMIR2019Sequences.h"

namespace musical {

double ISMIR2019OptiSimilarityRater::getScore(Sequences * seqs, int x1, int y1, int x2, int y2) const {
	...
	result = result * (1.0 - fabs(s2->phrasepos-s1->phrasepos)) * (1.0 - fabs(s2->imaweight-s1->imaweight));
